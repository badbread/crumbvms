...
| Key | Default | Notes |
|---|---|---|
| `MOTION_HWACCEL` | `cpu` | `cpu` (the default) forces software decode: it works on any host and is cheap at the ~320px/5fps analysis resolution. `vaapi` forces an Intel/AMD iGPU, `cuda` forces NVDEC, `auto` probes for NVDEC (note: the probe reports built-in ffmpeg support, not a usable runtime device, so on a GPU-less host it can wrongly pick cuda and break motion; prefer explicit `cpu`/`vaapi`/`cuda`). The admin console setting (`server_settings.motion_hwaccel`) overrides this, DB wins over env. Only affects motion decode; recording is always stream-copy |
