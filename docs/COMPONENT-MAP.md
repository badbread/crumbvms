...
| Recorder service | `services/recorder/src/` (`recording.rs`, `motion.rs`, `archive.rs`, `reconcile.rs`, `go2rtc_embed.rs`, `frigate_motion.rs`, `ha_motion.rs` (Home Assistant motion source: `motion_source='ha'` cameras triggered by linked HA sensors; writes labeled `'ha'` events; see `docs/DECISIONS.md` 2026-07-10), `decode_probe.rs`, `resource_stats.rs`) | Recording, motion detection, retention/eviction, segment index, embedded go2rtc supervision. The always-must-work component (golden rule 2) | `services/recorder/Dockerfile`; image `<prefix>/recorder:<version>` |
