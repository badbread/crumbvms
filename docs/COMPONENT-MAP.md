...
| Alert-text templates | `crumb_common::alert_template` (the `%token%` engine + built-in default templates), `system_alert_rules.message_template`/`title_template` (migration `0079`), rendered in `channel_notify::ChannelMessage::text_for()`/`rendered_title_for()`, resolved in `notifications.rs`; per-type token vocab surfaced by `SYS_ALERT_META_TOKENS` in `admin.html`. A new `event_key` that wants structured tokens ALSO populates `system_events.meta` at its emit site (via `db::insert_system_event_full`) and adds a `SYS_ALERT_META_TOKENS` entry | `NULL` template = built-in default (byte-identical to the legacy wording); built-in tokens (`%camera%`/`%event%`/`%detail%`/date-time) work for every type with no meta. Rendered text is JSON-escaped by the `serde_json` provider bodies, so a template can never break or inject into a payload |
| Timestamp rendering | `channel_notify::TimeStyle` + `time_style_for` pick the style from the channel `kind`; `ApiConfig::server_tz` (from `crumb_common::config::server_tz`, i.e. `TZ`) is threaded into `run_notification_engine`/`dispatch_system_events_tick` and carried on `ChannelMessage::tz`. Console copy: the token legend hint + `_sysAlertTokenSamples` in `admin.html`. Docs: `docs-site/docs/notifications/index.md`, `docs-site/docs/configuration/environment-reference.md` (`TZ`) | Discord and Slack render their own markup in each viewer's zone; every other provider gets the server zone with an abbreviation. A new channel kind with client-side markup adds a `TimeStyle` variant, it does not special-case a dispatcher (`docs/DECISIONS.md` 2026-09-08) |
