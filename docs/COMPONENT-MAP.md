...
| Response headers | `services/api/src/response_headers.rs` (`with_site_headers` + `admin_csp_layer`, both wired in `main.rs`) | `X-Content-Type-Options: nosniff` and `Referrer-Policy: no-referrer` on every response, applied outermost so the `/auth` subtree is covered too. The Content-Security-Policy is scoped to the `/admin` document and describes exactly what `admin.html` loads, so a console change that adds an external asset, an `<iframe>`, a `WebSocket` or a cross-origin `fetch()` must widen a directive in the same change (argue it in `docs/DECISIONS.md` first); `tests/response_headers.rs` asserts the wiring |
