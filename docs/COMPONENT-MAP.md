...
| Media tokens | scoped `?token=` mint (`auth.rs` media_token_routes) and every client's media-URL builder: android `MediaTokenCache.kt`/`MediaUrls.kt`, desktop `app.js`, iOS `Networking/` | Media must never carry the bearer JWT. A media token is only as weak as the WEAKEST guarantee of everything it can reach: an endpoint whose RESPONSE carries a credential, a long-lived URL, or more than the token's one camera must take `auth_mw::FullSessionUser`, not `AuthUser` (see DECISIONS 2026-08-06) |
