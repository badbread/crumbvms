...
| LPR plate report (single-plate PDF dossier, desktop-only; composed CLIENT-side from `GET /plates` + `GET /events/:id/snapshot` — there is no server report endpoint, and no server-side downscaled derivative to ask for, so sighting images are downscaled in-client before embedding) | Deferred (`docs/DECISIONS.md` 2026-07-31 plate-names entry lists the web report as a follow-up) | **Report** builder off a read row or the read pop-up: options form (date range, camera filter, order, image type + image count, page size, timezone, operator notes) -> in-app PDF **preview** with print and download (`plate_report_dialog.dart`, `plate_pdf_report.dart`, `plate_report_options.dart`); the option SHAPE persists via `PlatesPrefs`, the per-report bits (custom dates, camera selection, notes) never do | Deferred | Deferred |
