...
| **crumb-alpr worker** (opt-in) | `services/alpr-worker/` (`worker.py`, `Dockerfile`, `requirements.txt`, `README.md`) | Crumb-native LPR OCR (fast-alpr): pulls a camera's go2rtc restream, motion-gates, votes across a pass, and POSTs reads to `POST /lpr/reads` (which bridges into the same detection-ingester channel as Frigate). Polls `GET /lpr/worker-config` for zones/min-confidence. Opt-in `alpr` compose profile; one instance per camera; model weights fetched at first run (not vendored). See `docs/LPR-NATIVE-ENGINE-PLAN.md` + `docs/DECISIONS.md` 2026-07-17 | `services/alpr-worker/Dockerfile`; built locally via the `alpr` profile (no CI image yet) |
...
| Marketing site | `site/` (`index.html` hand-edited home, `privacy.html`, `updates/posts/*.md` authored, `updates/` + `feed.xml` + `sitemap.xml` + `data.json` generated by `site/scripts/build.mjs`, `styles/`, `app.js`, `assets/`) | Author a post in `updates/posts/`, run `node scripts/build.mjs` (Node 18+, zero deps), commit sources AND generated output. Copy rules: capability-first and humble tone, no em-dashes. Frigate and Home Assistant get equal weight in positioning slots (meta/og descriptions, hero, section headings, feature lists). Name other products only as a comparison ("if you have used X, this will feel familiar"), never as an identity ("the open source X"); no other product's logo or visual style; keep the trademark attribution line in the README/site footer current |
...
| AuthN/AuthZ | `services/api/src/auth.rs`, `auth_mw.rs`, `roles.rs` | Golden rule 1: every endpoint authenticated, through RBAC (admin vs role capabilities + per-camera grants). Media URLs use scoped short-lived `?token=` media claims, never the bearer JWT. Default to the plain `AuthUser` extractor, which rejects a media-scoped principal, so a new endpoint is safe by default; reaching for `MediaOrFullUser` instead is a security-review event (see the **Media tokens** row in section 2 G) |
...
| Channel management + destinations | `manage_channels` role capability (migration `0081`, default-off, admin implies, never on a media token) gates create/update/delete/test in `notifications.rs`; the destination check is `channel_notify::validate_channel_destinations` (syntactic, no DNS: http/https, non-empty host, no loopback/link-local/compose-service host for a non-admin, RFC1918 allowed); the test-fire is rate-limited per user by `AppState::count_channel_test`; the console mirrors the capability in `ROLE_CAP_FIELDS` and `NOTIF_CAN_MANAGE` | Listing channels stays open and existing channels keep delivering: only management is gated. The compose service-name list in `channel_notify.rs` must track `docker-compose*.yml` service names. See `docs/DECISIONS.md` 2026-07-20 (addendum) |
| Provider-supplied snapshot URL | `channel_notify::resolve_provider_snapshot_url`, applied at BOTH fetch sites (`events.rs::get_event_snapshot` and `notifications.rs::fetch_provider_snapshot`) | The value comes from a detection provider's MQTT payload, not an operator: a relative path joins onto the configured Frigate base, an absolute URL must sit on that base's own origin, anything else is not fetched |
...
| Copy rules | capability-first and humble tone, no em-dashes in drafted copy, name other products only as a comparison ("if you have used X, this will feel familiar") and never as an identity ("the open source X"), no other product's logo or visual style, LAN IPs/camera names in screenshots are fine | House style, non-negotiable |
...
### N. Server contract landed ahead of its renderers (a deliberate wave split)

Use when a schema + wire field ships on its own so several clients can then
implement against a frozen, already-merged contract (wave A = server, wave B =
renderers). This is NOT a licence to forget the clients: an additive field no
client reads is dead weight, so the deferral must be named and tracked.

| Surface | Path | Why |
|---|---|---|
| Rows A + C | the API/DTO and migration checklists above | A wave-A change is always both of those; run them in full |
| Contract statement | the `docs/DECISIONS.md` entry (row M) must state the resolution rule the renderers will implement, verbatim | The whole point of splitting is that wave B has one unambiguous thing to code against; a rule that lives only in a PR description is lost |
| Every read path | every SELECT/RETURNING list and row-mapper that builds the affected type in `services/common/src/db.rs`, plus any carry-over snapshot (e.g. `replace_camera_ha_links` preserves placements across a link re-save) | A field loaded in some paths and not others reads as null intermittently, which looks like data loss to an operator |
| Backward compatibility | new field additive and nullable; the null/default must reproduce the PREVIOUS rendering exactly | Old clients ignore the field and must keep behaving as before, in both directions of the version skew |
| Deferral record | the section 3 parity row names the shipping surfaces and the ones still to come, with the tracking issue | Section 3's rule: "the others come later" is fine, "the others were forgotten" is not |
...
| Feature area | Web admin (`services/api/src/admin.html`) | Desktop (`apps/desktop-flutter/lib/`) | Android (`apps/android/.../feature/`) | iOS/macOS (`apps/ios/Crumb/Features/`) |
|---|---|---|---|---|
...
| License plates (LPR, `docs/DECISIONS.md` 2026-07-13 + 2026-07-17 single-control entry; backend: `plates.rs` `GET /plates` + admin `GET /lpr/storage`, `plate_reads`/`lpr_config` migration `0051`, per-camera engine/zones migrations `0069`/`0071` (engine `none`/`frigate`/`crumb-alpr`/`both` is the single per-camera control, `lpr_enabled` derived), Frigate ingest in `detection/frigate.rs` gated per-camera in `detection_ingester.rs`, `view_plates` cap) | Dedicated **LPR** section: global enable/retention (+ storage-usage hint) + ingest-token rotate, per-camera engine table + draw-and-edit zone editor, watchlist + reads (reads/watchlist gated on enabled) | **Phase 0**: "Plates" tab (gated on `MeResponse.plates_enabled`), list → click-to-playback, copy-plate-to-clipboard from a row / the read pop-up (the Flutter client `apps/desktop-flutter/`) | **"Plates" tab** (gated on `plates_enabled`): reads list + plate bbox + single-plate PDF report (BOLO banner, images, and an optional sighting-history dossier: a bounded thumbnail strip plus a paginated **full sighting log** listing every held sighting's date/time + camera) with a post-generate **Open / Save to Downloads / Share** chooser (`feature/plates/PlatesScreen.kt`, `PlatesViewModel.kt`, `PlateBbox.kt`, `PlatesPdf.kt`; Downloads save mirrors `feature/export/ExportScreen.kt` — MediaStore on API 29+, legacy public Downloads on ≤28) | **"Plates" tab** (gated on `plates_enabled`): reads list (search + match modes, fuzzy order preserved), duplicate-read collapse (×N), plate-crop thumbnails, click-to-playback (`apps/ios/Crumb/Features/Plates/`) |
| LPR alerts / watchlist (LPR Phase 2, `docs/DECISIONS.md` 2026-07-13; backend: `lpr_watchlist` migration `0052`, `/lpr/watchlist` GET `view_plates` / POST+DELETE admin-only in `plates.rs`, match+emit in `detection_ingester.rs`, `plate_watchlist_hit` event_key rides `system_events`/`notifications.rs` fan-out) | Plates → **Watchlist** manager (add/remove/notify) + `plate_watchlist_hit` in Notifications → System alerts | "Plates" tab watchlist manager + add-to-watchlist from a read | Phase 2 client watchlist manager + add-to-watchlist from a read | Watchlist manager (add/edit incl. watch/ignore kind, remove) + add-to-watchlist from a read + client-side fuzzy match (`Lpr` matcher) |
| LPR copy plate to clipboard (client-only affordance, no backend change: the RAW plate string is copied, never the resolved `display_name`) | Per-row **Copy** button on LPR reads + Watchlist rows (`copyPlate` → the existing `copyText`, which falls back to `document.execCommand('copy')` because the console is normally served over plain HTTP on a LAN address and `navigator.clipboard` is undefined outside a secure context) | Tracked separately (not in the console/Android change) | Copy icon button beside the raw plate on every reads view (list, gallery, grouped, timeline) in `feature/plates/PlatesScreen.kt`; snackbar confirmation only below API 33, where the system does not show its own clipboard panel. **Deferred:** the watchlist manager rows (a modal, so the Scaffold snackbar would not be visible) | Tracked separately (not in the console/Android change) |
