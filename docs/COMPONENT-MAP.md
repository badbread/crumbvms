...
| Marketing site | `site/` (`index.html` hand-edited home, `privacy.html`, `updates/posts/*.md` authored, `updates/` + `feed.xml` + `sitemap.xml` + `data.json` generated by `site/scripts/build.mjs`, `styles/`, `app.js`, `assets/`) | Author a post in `updates/posts/`, run `node scripts/build.mjs` (Node 18+, zero deps), commit sources AND generated output. Copy rules: capability-first and humble tone, no em-dashes. **Frigate and Home Assistant get equal weight in positioning slots** (meta/og descriptions, hero, section headings, feature lists, taglines): they are the two integrations a self-hosting reader already runs. Equal *prominence*, not equal mention counts, since Frigate is more entangled architecturally (MQTT contract, detection source, restream) and body text will always name it more. Name other products as a **comparison** ("if you have used X, this will feel familiar"), never as an **identity** ("the open source X"). Use no other product's logo or visual style, claim no compatibility that does not exist, and keep the trademark attribution line in the README/site footer current |
