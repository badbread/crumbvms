...
Tested end to end as an in-place upgrade from the published v0.1.1 images. It is
a drop-in: `.env` needs no changes, the new migrations apply in one pass on first
boot, footage came through byte-identical, and logins, roles, policies, and the
authenticated RTSP restream default were intact afterwards.
...
- **`MOTION_HWACCEL`:** a new install now defaults to `cpu`, but an `.env`
  ...
  compiled into ffmpeg (true on every host running the published image, which is
  what made it pick a GPU that was not there); it opens a real cuda device at
  startup and resolves to `cpu` when that fails, times out, or cannot start, and
  any backend that decodes no frames is demoted to CPU for that camera. To adopt
  the new default anyway, set `MOTION_HWACCEL=cpu` and `docker compose up -d`, or
  clear the console's hardware-decode field and set it there. If you have a GPU
  you want used, name it (`cuda` or `vaapi`) rather than relying on `auto`.
- **Non-admin roles cannot control Home Assistant devices** until you grant them
  ...
  existing role, so nobody gains the ability to unlock a door by upgrading.
  Admins are unaffected.
- **Motion health is a new surface**, so cameras that were quietly degraded
  before will start saying so. That is the feature working.
- **Take a database backup first** ([docs/BACKUP.md](BACKUP.md)), as with any
  upgrade.
- **Rolling back is reasoned, not rehearsed.** The new migrations are additive,
  so pinning the previous `CRUMB_VERSION` and running `docker compose up -d`
  should bring 0.1.1 back up against the upgraded schema: you lose the new
  features, not the footage. Nobody has actually executed that downgrade, so
  treat it as an argument rather than a tested procedure, and rely on the backup.
